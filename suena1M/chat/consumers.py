...
class ChatConsumer_async(AsyncWebsocketConsumer):
    ...
    async def chat_message(self, event):
        ...
        await self.send(text_data=json.dumps({"message": f"{player_name}: {message}"}))
...
class ChatConsumer_sync(WebsocketConsumer):
    def connect(self):
        self.room_name = self.scope["url_route"]["kwargs"]["room_name"]
        self.room_group_name = "chat_%s" % self.room_name
